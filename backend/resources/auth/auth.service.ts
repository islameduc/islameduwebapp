...
class AuthService {
    ...
    public async getAboutUs(req: Request): Promise<object | Error> {

        try {
            const data = await DetailModel.find({ isDeleted: false })
                .exec()
            return data
        } catch (error: any) {
            throw new HttpException(
    ...
    public async addAboutUsDetail(req: Request): Promise<Detail | Error> {
        try {
            ...
            const existingDetail = await DetailModel.findOne({
                title,
                isDeleted: false
            }).exec()

            if (existingDetail) {
                throw new HttpException(409, `A detail already exists with the title ${title}`)
            }

            const detail = await new DetailModel({
                title,
                content,
                author: userId
            }).save()
    ...
    public async updateAboutUs(req: Request): Promise<object | Error> {
        try {
            const userId = req.user._id
            const { body } = req
            const whoWeAre = body.whoWeAre
            const mission = body.mission
            const vision = body.vision
            const whatWeDo = body.whatWeDo

            await DetailModel.findOneAndUpdate({ title: 'whoWeAre' }, { content: whoWeAre, author: userId })
            await DetailModel.findOneAndUpdate({ title: 'mission' }, { content: mission, author: userId })
            await DetailModel.findOneAndUpdate({ title: 'vision' }, { content: vision, author: userId })
            await DetailModel.findOneAndUpdate({ title: 'whatWeDo' }, { content: whatWeDo, author: userId })
