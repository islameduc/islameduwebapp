            ...
            try {
                yield mongoose_1.default.connect(String(process.env.MONGO_URI)).then(() => __awaiter(this, void 0, void 0, function* () {
                    ...
                    this.httpServer.listen(process.env.PORT, () => {
                        console.log(`IPES Server running on port http://localhost:${this.port} and on ${process.env.NODE_ENV} mode`);
                    });
                }));
                if (process.env.NODE_ENV !== 'production') {
                    yield (0, utils_1.initializeDevelopmentDummyData)();
                }
                console.log('IPES MongoDB Connection Success 👍');
            }
            catch (err) {
                console.log('IPES MongoDB Connection Failed 💥');
                console.log(err);
                process.exit(1);
            }
        });
